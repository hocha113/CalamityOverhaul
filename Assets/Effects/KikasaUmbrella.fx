...
//TechTrail:墨痕拖尾(世界空间 TriangleStrip,vs+ps):被气流从伞面剥离的湿墨,
//          中实边蚀、尾端阈值抬升撕成丝缕、轴心一线血芯尾部先断、新段窄湿光
//坐标全笛卡尔;直线算术+普通 tex2D,FNA3D 安全;预乘输出进 AlphaBlend 批
//s0=物品贴图(TechFill/TechTrail 不采样) s1=PerlinNoise
//消费入口 KikasaRains/KikasaRainUmbrella.cs + KikasaRainRender.cs
// ============================================================================

sampler uImage0 : register(s0);
sampler uImage1 : register(s1);

float4x4 transformMatrix;   //TechTrail 专用:世界坐标顶点直入
...
float4 PSTrail(PSInput input) : COLOR0 {
    ...
    //PerlinNoise 实测值域 0.227~0.776,阈值前先归一
