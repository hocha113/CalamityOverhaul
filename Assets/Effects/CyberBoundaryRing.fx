...
//  L3 环被噪声侵蚀解体，内侧长出向域心流动的数据带(世界正被吸入读取)
//
...
float3 tierWeights;     //三层几何权重(方格/蜂巢/流场)
...
float4 PixelShaderFunction(float2 coords : TEXCOORD0, float4 vertexColor : COLOR0) : COLOR0
{
    ...
    float noiseDisp = (n1 * 0.6 + n2 * 0.4 - 0.5) * (0.026 + 0.014 * w3);

    float adjDist = dist + noiseDisp;
    float signedOut = adjDist - ringProgress;   //>0 在环外侧

    //主环遮罩：L1 收细，L3 略厚
    float thick = ringThickness * (0.55 * w1 + 1.0 * w2 + 1.15 * w3);
    ...
    //L3：环体噪声侵蚀，断弧缓慢漂移
    float erosion = lerp(1.0, smoothstep(0.28, 0.60, n2), w3 * 0.60);
    ringMask *= erosion;
    ...
    float insideAmt = saturate(-signedOut / (thick * 5.0));
    ...
    float3 ringColor = lerp(coreRed, hotEdge, edgeBlend * 0.6) * (ringMask * 0.80);

    float3 tongueColor = lerp(emberDark, coreRed, tongueFade) * tongue * 0.55;
    float3 bandColor = float3(1.0, 0.30, 0.12) * inwardBand * 0.50;
    float3 tickColor = float3(0.95, 0.30, 0.16) * tickGlow * 0.45;
    float3 cornerColor = float3(1.0, 0.50, 0.24) * cornerGlow * 0.55;

    //合成
    float3 finalColor = (ringColor + tongueColor + bandColor + tickColor + cornerColor) * fadeAlpha;
    float alpha = saturate(ringMask * 0.70 + tongue * 0.30 + inwardBand * 0.30
