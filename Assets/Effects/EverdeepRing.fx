...
float4 PixelShaderFunction(float2 uv : TEXCOORD0, float4 vColor : COLOR0) : COLOR0
{
    ...
    // ---- 环几何:噪声蚀刻厚度,轮廓本身随循环转动 ----
    float edgeN = nrm(tex2D(noiseSamp, Rot(p, spin * 0.6) * 0.85 + uSeed).g);
    float th = 0.135 * (0.72 + edgeN * 0.62);
    float d = abs(r - 0.54);
    float band = smoothstep(th, th * 0.30, d);
    ...
    float flecks = fleckZone * smoothstep(0.70, 0.86, fleckN) * (0.30 + tail * 1.15);

    // ---- 内缘生物光脉络:一线青辉沿内缘游动,充能时炽亮 ----
    float veinD = abs(r - (0.54 - th * 0.60));
    float vein = smoothstep(0.042, 0.0, veinD) * (0.35 + band * 0.65)
        * (0.45 + 0.55 * sin(theta * 3.0 + spin * 1.7 + uSeed * 9.0))
