...
namespace CalamityOverhaul.Content.Items.Melee.WeaverGrievanceses
{
    ...
    internal sealed class WGManifestationSystem : ModSystem
    {
        ...
        public override void LoadWorldData(TagCompound tag) {
            ...
            try {
                bool currentSave = tag != null && tag.TrySafeGet(SaveVersionKey, out int version)
                    && version >= SaveVersion;
                string unlockedKey = currentSave ? UnlockedKey : nameof(Unlocked);
                Unlocked = tag != null && tag.TrySafeGet(unlockedKey, out bool unlocked) && unlocked;
                if (Unlocked) {
        ...
        private static void LoadPersistentState(TagCompound tag, bool currentSave) {
            ...
            Vector2 legacyAnchor = tag.TrySafeGet("Anchor", out Vector2 anchor)
                ? anchor : Vector2.Zero;
            ManifestationCompleted = tag.TrySafeGet(nameof(ManifestationCompleted), out bool legacyCompleted)
                && legacyCompleted;
            if (ManifestationCompleted) {
