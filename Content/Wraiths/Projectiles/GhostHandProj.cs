...
namespace CalamityOverhaul.Content.Wraiths.Projectiles
{
    ...
    internal sealed class GhostHandProj : ModProjectile
    {
        ...
        //==== 骨架（与夺身处决演出共用，见 GhostHandRig）====
        private const int ArmSegmentCount = GhostHandRig.ArmSegmentCount;
        private const float SegmentLength = 52f;
        private const float MaxReach = ArmSegmentCount * SegmentLength;
        private readonly GhostHandRig rig = new();
        ...
        private void SnapToHandSlot() {
            ...
            rig.Identity = Projectile.identity;
            rig.Snap(Projectile.Center, shoulderPos);
        }
        ...
        private void UpdateArmIK() {
            rig.Identity = Projectile.identity;
            Projectile.Center = rig.SolveArm(shoulderPos, Projectile.Center, armTension, ownerDirection);
        }

        private void UpdateFingers() => rig.SolveFingers(fingerCurl, fingerCurlOffsets);
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            var armVerts = rig.BuildArmStrip(gripBlend, Main.GlobalTimeWrappedHourly);
            var palmVerts = rig.BuildPalmStrip(gripBlend);
