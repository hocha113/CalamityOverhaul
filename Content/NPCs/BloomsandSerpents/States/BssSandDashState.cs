...
namespace CalamityOverhaul.Content.NPCs.BloomsandSerpents.States
{
    ...
    internal class BssSandDashState : BssStateBase
    {
        ...
        private void UpdateWindup(BssStateContext ctx, NPC npc) {
            ...
            //贴地尘线：线下沙面被吸气掀起（各端本地，给预警线一层沙材质的落地感）
            if (!Main.dedServ && t > 2 && Main.GameUpdateCount % 2 == 0) {
                int puffs = 1 + (int)(progress * 2f);
                for (int i = 0; i < puffs; i++) {
                    float along = Main.rand.NextFloat(60f, 200f + 600f * progress);
                    ...
                    Dust d = Dust.NewDustPerfect(pos, DustID.Sand,
                        new Vector2(lockedDir.X * 1.2f, -Main.rand.NextFloat(0.6f, 1.6f) * (0.5f + progress)),
        ...
        /// <summary>硬刹：×0.66/帧的急停沙浪，连段未满回就位</summary>
