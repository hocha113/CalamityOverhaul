    ...
    internal class BssTailSweepState : BssStateBase
    {
        ...
        private void UpdatePass(BssStateContext ctx, NPC npc) {
            ...
            if (passedBy > BssDirector.SweepOvershoot || Timer >= BssDirector.SweepPassFrames) {
                //入弯：拉升点锁定（先上拔再回瞄，弧顶约等于转弯半径，链条沿弧铺成鞭）
                turnLift = npc.Center + passDir * BssDirector.SweepTurnLiftX + new Vector2(0f, -BssDirector.SweepTurnLiftY);
                ctx.PulseWhip(14f);
                if (!Main.dedServ) {
        ...
        private void UpdateTurn(BssStateContext ctx, NPC npc) {
            ...
            ctx.MoveTarget = t < 14 ? turnLift : PredictTarget(ctx, 8f);
            ctx.MoveSpeed = 26f;
            ctx.TurnRadius = BssDirector.SweepTurnRadius;
            ctx.AccelRate = 0.18f;
            ctx.LegCommand = BssLegCommand.Flail;
            DeclareJaw(ctx, BssJawCommand.Gape);
        ...
        private void FlingNeedles(BssStateContext ctx, NPC npc) {
            ...
            foreach (var seg in ctx.Segments) {
                ...
                Projectile.NewProjectile(npc.GetSource_FromAI(), seg.Center + dir * (12f * seg.scale),
