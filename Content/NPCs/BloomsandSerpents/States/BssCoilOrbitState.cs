    ...
    internal class BssCoilOrbitState : BssStateBase
    {
        ...
        public override IBssState OnUpdate(BssStateContext ctx) {
            ...
            if (t < ExitFrame) {
                ...
                ctx.MoveSpeed = 19f;
                ctx.TurnRadius = BssDirector.OrbitHookRadius;
                ctx.AccelRate = 0.2f;
                ctx.LegCommand = BssLegCommand.Tuck;
                ctx.BloomGlow = Math.Max(ctx.BloomGlow, 1f);
                ctx.GatherLevel = MathHelper.Clamp(hookFrames / 14f, 0f, 1f);
                DeclareJaw(ctx, BssJawCommand.Inhale, ctx.GatherLevel);

                //切向 ⊥ 径向：90° 的弧在半径 260、速度 19 下约 22 帧走完，窗口给到 28
