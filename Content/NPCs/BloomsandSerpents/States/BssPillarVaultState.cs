...
{
    /// <summary>
    /// 沙柱腾跃：选最近滞留柱（无柱先召一根）→ 贴近柱脚 → 沿柱面扫着爬上去（腿抓柱身、
    /// 蓄力聚拢上膛；106 宽的身体绕不了 96 宽的柱，横摆小而慢、头锁向上，链从柱脚一路挂上来）
    /// → 柱顶盘紧静止一拍（收势即预告）→ 蹬柱上抛跳到空中（柱身同帧
    ...
    internal class BssPillarVaultState : BssStateBase
    {
        ...
        private void UpdateClimb(BssStateContext ctx, NPC npc) {
            ...
            //头沿柱面向上爬：朝向锁向上、随横摆轻倾，不跟着伺服速度左右甩头
            float lean = MathHelper.Clamp((desired.X - npc.Center.X) * 0.02f, -0.45f, 0.45f);
            ctx.AimAngle = new Vector2(lean, -1f).ToRotation();
