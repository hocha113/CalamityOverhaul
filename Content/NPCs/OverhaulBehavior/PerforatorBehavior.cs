        ...
        public virtual void Intensive(NPC npc)
        {
            ...
            {//一阶段
                if (npc.localAI[0] != 0)
                {
                    ...
                    {
                        if (npc.localAI[0] % 180 == 0)
                        {//在一阶段，我们需要周期性的发射横跨整个屏幕的水平弹幕来提高难度
            ...
            {//在二阶段
                if (!CWRUtils.isClient)
                {
                    if (npc.localAI[0] % 90 == 0 && npc.localAI[0] != 0 && lifeRatio < 0.4f)
                    {//周期性发射圆周性的灵液球弹幕
                        for (int i = 0; i < 16; i++)
                        {
                ...
                if (npc.localAI[0] % 150 == 0 && npc.localAI[0] != 0)
                {//我们让它每隔一小段时间就瞬移一次
