...
{
    /// <summary>腐蚀唾液团；ai[0]模式 0抛射团 1酸雨滴 2喷发重团；落地留酸池(雨滴不留)</summary>
    internal class EowAcidGlob : ModProjectile
    {
        ...
        private float Gravity => IsRainDrop ? 0.42f : IsHeavy ? 0.3f : 0.24f;
        ...
        public override void AI() {
            ...
            if (!VaultUtils.isServer && !IsRainDrop && Main.rand.NextBool(6)) {
                PRTLoader.NewParticle<PRT_AcidSplash>(
                    ...
                    Color.White, Main.rand.NextFloat(0.3f, 0.55f)).Configure(Main.rand.Next(14, 24));
            }

            Lighting.AddLight(Projectile.Center, EowMotionFX.AcidGreen.ToVector3() * (IsHeavy ? 0.6f : 0.35f));
        }

        public override void OnKill(int timeLeft) {
            //落点酸池：权威端生成(雨滴不留池)
            if (!VaultUtils.isClient && !IsRainDrop) {
                Vector2 ground = EowMotionFX.FindGroundBelow(Projectile.Center - new Vector2(0, 8f));
                //落点离地太远(打在墙上/半空消散)则不留池
                if (Vector2.Distance(ground, Projectile.Center) < 120f) {
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), ground, Vector2.Zero,
                        ModContent.ProjectileType<EowAcidPool>(),
                        (int)(Projectile.damage * 0.55f), 0f, Main.myPlayer, IsHeavy ? 1f : 0f);
                }
            }
            ...
            //酸沫飞溅
            float power = IsHeavy ? 1.3f : IsRainDrop ? 0.45f : 0.85f;
            EowMotionFX.SpawnAcidBurst(Projectile.Center, power, -Projectile.velocity.SafeNormalize(Vector2.Zero) * 3f);
            if (!IsRainDrop) {
                SoundEngine.PlaySound(SoundID.Item54 with { Volume = 0.5f, Pitch = 0.3f, MaxInstances = 5 }, Projectile.Center);
            }
        }
        ...
        /// <summary>着色器路径：翻转批到 Immediate 画酸团quad</summary>
        ...
        /// <summary>精灵回退：深底+亮膜+高光点三层</summary>
        private void DrawSpriteGlob(Vector2 drawPos, float stretch, float fade) {
            ...
            for (int i = Projectile.oldPos.Length - 1; i >= 0; i--) {
                ...
                Main.EntitySpriteDraw(tex, pos, null, EowMotionFX.AcidDeep with { A = 40 } * (0.3f * t * fade),
                    Projectile.rotation, origin, scaleVec * (0.7f * t), SpriteEffects.None, 0);
            }

            Main.EntitySpriteDraw(tex, drawPos, null, EowMotionFX.AcidDeep with { A = 160 } * fade,
                Projectile.rotation, origin, scaleVec, SpriteEffects.None, 0);
            Main.EntitySpriteDraw(tex, drawPos, null, EowMotionFX.AcidGreen with { A = 90 } * (0.9f * fade),
                Projectile.rotation, origin, scaleVec * 0.72f, SpriteEffects.None, 0);
            Main.EntitySpriteDraw(tex, drawPos + new Vector2(-2f, -3f), null,
                EowMotionFX.AcidBright with { A = 0 } * (0.55f * fade),
