...
namespace CalamityOverhaul.Content.NPCs.HeavenEaters
{
    internal class HEHead : ModNPC
    {
        ...
        public const string Path = "CalamityOverhaul/Assets/NPCs/HeavenEaters/";

        public static int phaseIconIndex;

        public override string Texture => Path + "HEHead";
        ...
        //在他们听不到声音电报之前距离目标的最大距离
        private const float soundDistance = 2800f;

        // 长度的量
        private const int minLength = 70;
        private const int maxLength = 71;
        ...
        public static void LoadHaedIcon() {
            string phaseIconPath = "CalamityMod/NPCs/DevourerofGods/DevourerofGodsHead_Head_Boss";
            CWRMod.Instance.AddBossHeadTexture(phaseIconPath, -1);
            phaseIconIndex = ModContent.GetModBossHeadSlot(phaseIconPath);
        }
        ...
        public override void SetStaticDefaults() {
            ...
            NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers(0)
            {
                ...
                CustomTexturePath = Path + "HE_BossChecklist"
            };
            value.Position.X += 55;
            value.Position.Y += 5;
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]{
                ...
                new FlavorTextBestiaryInfoElement("Mods.CalamityMod.Bestiary.HE")
            });
        }
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient) {
                if (!TailSpawned && NPC.ai[0] == 0f) {
                    ...
                    for (int i = 0; i < maxLength; i++) {
                        ...
                        if (i >= 0 && i < minLength) {
                            if (i % 2 == 0)
                                lol = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<HEBody>(), NPC.whoAmI);
                            else
                                lol = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<HEBodyAlt>(), NPC.whoAmI);
                        }
                        else
                            lol = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<HETail>(), NPC.whoAmI);
                ...
                if (player.dead) {
                    ...
                    if (NPC.position.Y > Main.rockLayer * 16.0) {
                        for (int a = 0; a < Main.maxNPCs; a++) {
                            if (Main.npc[a].type == NPC.type || Main.npc[a].type == ModContent.NPCType<HEBodyAlt>() || Main.npc[a].type == ModContent.NPCType<HEBody>() || Main.npc[a].type == ModContent.NPCType<HETail>())
            ...
            switch ((int)AIState) {
                ...
                case (int)Phase.LightningRain:
                    ...
                    if ((destination - NPC.Center).Length() < lightningRainLocationDistance || calamityGlobalNPC.newAI[2] > 0f) {
                        ...
                        CWRDust.SpanCycleDust(NPC, DustID.Electric, DustID.Adamantite);
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient) {
                        if (calamityGlobalNPC.newAI[2] == 0f) {
                            if (!NPC.AnyNPCs(ModContent.NPCType<CeaselessVoid>())) {
                                    NPC.SpawnOnPlayer(NPC.FindClosestPlayer(), ModContent.NPCType<CeaselessVoid>());
                            }
                            if (!NPC.AnyNPCs(ModContent.NPCType<StormWeaverHead>())) {
                                NPC.SpawnOnPlayer(NPC.FindClosestPlayer(), ModContent.NPCType<StormWeaverHead>());
                            }
                            if (!NPC.AnyNPCs(ModContent.NPCType<Signus>())) {
                                NPC.SpawnOnPlayer(NPC.FindClosestPlayer(), ModContent.NPCType<Signus>());
                            }
                        }
                    }
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot) {
            npcLoot.Add(ModContent.ItemType<Earth>());
            npcLoot.Add(ModContent.ItemType<TriactisTruePaladinianMageHammerofMightMelee>());
            npcLoot.Add(ModContent.ItemType<DraconicDestruction>());

            npcLoot.Add(ModContent.ItemType<Contagion>());
            npcLoot.Add(ModContent.ItemType<Voidragon>());
            npcLoot.Add(ModContent.ItemType<HalibutCannon>());

            npcLoot.Add(ModContent.ItemType<ScarletDevil>());
            npcLoot.Add(ModContent.ItemType<TemporalUmbrella>());
            npcLoot.Add(ModContent.ItemType<Vigilance>());

            npcLoot.Add(ModContent.ItemType<TerminusOver>());
            npcLoot.Add(ModContent.ItemType<TextItem>());
        }
        ...
        public override void HitEffect(NPC.HitInfo hit) {
            ...
            if (NPC.life <= 0) {
                for (int k = 0; k < 15; k++)
                    Dust.NewDust(NPC.position, NPC.width, NPC.height, 4, hit.HitDirection, -1f, 0, default, 1f);
                Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModLoader.GetMod("CalamityMod").Find<ModGore>("DoT").Type, 1f);
                //Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("HE").Type, 1f);
            }
        }
