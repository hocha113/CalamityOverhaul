...
{
    /// <summary>TBUG 对话/商店 HUD 原语；fork 自 VictorUIStyle，配色换黑绿终端</summary>
    internal static class TBUGUIStyle
    {
        ...
        public static int DrawCommandRow(SpriteBatch sb, Rectangle rect, Color accent, float hoverT, float alpha, bool separator = true) {
            ...
            if (hoverT > 0.001f) {
                ...
                for (int i = 0; i < strips; i++) {
                    ...
                    Color c = accent * (alpha * hoverT * 0.28f * (1f - t));
                    sb.Draw(px, new Rectangle(r.X + (int)(r.Width * t), r.Y, r.Width / strips + 1, r.Height), c);
                }
            }
            else {
                //空闲淡底
                sb.Draw(px, new Rectangle(r.X, r.Y, r.Width, r.Height), TBUGTheme.RowBg * (alpha * 0.35f));
            }
        ...
        public static void DrawPrice(SpriteBatch sb, Vector2 pos, long value, float alpha, float scale, bool rightAlign, string freeText = "FREE") {
            if (value <= 0) {
                ...
                Utils.DrawBorderString(sb, freeText, new Vector2(rightAlign ? pos.X - fs.X : pos.X, pos.Y), TBUGTheme.AccentAmber * alpha, scale);
                return;
            }
