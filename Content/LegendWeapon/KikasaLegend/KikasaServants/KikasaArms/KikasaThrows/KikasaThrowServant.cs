...
namespace CalamityOverhaul.Content.LegendWeapon.KikasaLegend.KikasaServants.KikasaArms.KikasaThrows
{
    /// <summary>
    /// 械奴·湖水掷手（通用掷奴）。单弹幕驱动至多三名掷手，各托一枚湖水凝成的投掷物：
    /// 轮转投掷走后仰旋蓄-掷出-再凝三拍（掷出的是原武器登记的真弹幕——
    /// 消耗品的弹幕结构上必须自含，转发安全性由此背书；手里那枚只是凝出来的影，
    /// 掷空后湖水再凝一枚）。隔两次散花齐掷：三掷手各泼一把扇面。
    /// 联机契约与通用械奴同构：owner 裁决转场、弹只在 authority 生成、
    /// 节拍闩防快照回卷、掷手数与武器类型 spawn 后经 ExtraAI 随包补发
    /// </summary>
    internal class KikasaThrowServant : ModProjectile, IKikasaArmsServant
    {
        public override string Texture => CWRConstant.VaultPlaceholder;

        //==================== 可调基数（占位初值，验收再调）====================

        /// <summary>掷弹基伤（召唤加成与档案倍率前）</summary>
        internal const int ThrowDamage = 165;
        ...
        private int armsItemType = ItemID.Shuriken;

        public int ArmsItemType => armsItemType;

        private KikasaThrowProfile? profileCache;

        private KikasaThrowProfile Profile => profileCache ??= KikasaArmsProfiler.ThrowProfileOf(armsItemType);
        ...
        //轮转投掷：逐手轮值——后仰旋蓄→掷出→再凝
        private const int ThrowWindup = 10;
        ...
        //散花齐掷：拢势→逐手错帧泼扇面
        private const int FanWindup = 20;
        private const int FanStagger = 5;
        private const int FanTotal = 66;

        //再凝时长：掷空后手里那枚重新凝出来
        private const int RefillFrames = 14;
        ...
        /// <summary>再凝计时：>0 表示手里是空的，正在凝下一枚</summary>
        private readonly int[] refill = new int[MaxHands];
        ...
        internal static void Summon(Player owner, Vector2 emergeAt, int count, int itemType) {
            ...
            KikasaThrowProfile profile = KikasaArmsProfiler.ThrowProfileOf(itemType);
            count = Math.Clamp(count, 1, profile.MaxUnits);
            int damage = (int)owner.GetTotalDamage(DamageClass.Summon).ApplyTo(ThrowDamage * profile.ThrowDamageMul);
            int index = Projectile.NewProjectile(owner.GetSource_Misc("KikasaServant"),
                emergeAt + new Vector2(0f, 42f), Vector2.Zero,
                ModContent.ProjectileType<KikasaThrowServant>(), damage, 2f, owner.whoAmI);
            if (index >= 0 && index < Main.maxProjectiles
                && Main.projectile[index].ModProjectile is KikasaThrowServant pack) {
        ...
        public override void AI() {
            ...
            Projectile.timeLeft = 180;
            Projectile.damage = (int)owner.GetTotalDamage(DamageClass.Summon).ApplyTo(ThrowDamage * Profile.ThrowDamageMul);
            ...
            switch (State) {
                ...
                case StateFan: UpdateFan(owner, authority); break;
        ...
        private void UpdateEmerge(Player owner, KikasaDomainPlayer domain) {
            ...
            for (int i = 0; i < handCount; i++) {
                if (!breachDone[i] && t >= BreachTime(i)) {
                    ...
                    handSpin[i] = (i % 2 == 0 ? 1f : -1f) * 0.4f;
                    if (i == 0) {
                        Projectile.velocity = new Vector2(0f, -3f);
                    }
                    SoundEngine.PlaySound(SoundID.SplashWeak with {
                        Volume = 0.7f, Pitch = -0.38f + i * 0.07f, MaxInstances = 3
                    }, handPos[i]);
                    if (viewed) {
        ...
        private void UpdateFollow(Player owner, bool authority) {
            ...
            if (target >= 0 && attackCooldown <= 0 && StateTimer > 30) {
                ...
                StateTimer = 0;
                StateParam = 0;
                State = attackIndex % 3 == 0 ? StateFan : StateRelay;
                Projectile.netUpdate = authority;
            }
        }
        ...
        private void UpdateRelay(Player owner, bool authority) {
            ...
            Vector2 focus = target >= 0
                ? Main.npc[target].Center + Main.npc[target].velocity * (300f / Profile.ThrowSpeed)
            ...
            if (duty < handCount) {
                ...
                if (p == 2) {
                    SoundEngine.PlaySound(SoundID.Item7 with {
                        Volume = 0.24f, Pitch = 0.2f, MaxInstances = 3
                    }, handPos[duty]);
                }
                if (p == ThrowWindup && duty > lastFireTick[duty]) {
                    lastFireTick[duty] = duty;
                    ThrowOne(owner, authority, duty, focus, 0f, 1f);
                }
            }
        ...
        /// <summary>掷出一枚：转发原武器登记的弹幕，手里的影掷空再凝</summary>
        private void ThrowOne(Player owner, bool authority, int i, Vector2 focus, float skew, float mul) {
            Vector2 from = handPos[i];
            Vector2 aim = (focus - from).SafeNormalize(Vector2.UnitX).RotatedBy(skew);
            refill[i] = RefillFrames;
            handSpin[i] = 0.55f * (aim.X >= 0f ? 1f : -1f);
            handVel[i] -= aim * 2.2f;

            SoundEngine.PlaySound(Profile.ThrowSound with {
                Volume = 0.36f, Pitch = 0.1f + i * 0.05f, MaxInstances = 4
            }, from);
            if (!Main.dedServ) {
                for (int k = 0; k < 3; k++) {
            ...
            if (authority) {
                int damage = (int)owner.GetTotalDamage(DamageClass.Summon)
                    .ApplyTo(ThrowDamage * Profile.ThrowDamageMul * mul);
                //出手轻抬：重弹（手雷类）借抛物补一点
                Vector2 vel = aim * Profile.ThrowSpeed;
                vel.Y -= 1.4f;
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), from, vel,
                    Profile.ThrowProjType, damage, 3f, Projectile.owner);
            }
        }

        //==================== 散花齐掷 ====================

        private void UpdateFan(Player owner, bool authority) {
            int t = (int)StateTimer;
            int target = FindTarget(owner);

            if (target < 0 && t <= FanWindup / 2) {
                EndAttack(authority, 60);
                return;
            }
            Vector2 focus = target >= 0
                ? Main.npc[target].Center + Main.npc[target].velocity * (260f / Profile.ThrowSpeed)
                : Projectile.Center + new Vector2(owner.direction * 300f, 0f);
            ...
            if (t == 6) {
                SoundEngine.PlaySound(SoundID.Item7 with {
                    Volume = 0.32f, Pitch = -0.1f, MaxInstances = 2
                }, Projectile.Center);
            }

            //逐手错帧泼扇面：一手三枚
            for (int i = 0; i < handCount; i++) {
                if (t == FanWindup + i * FanStagger && 0 > lastFireTick[i]) {
                    lastFireTick[i] = 0;
                    for (int k = 0; k < 3; k++) {
                        float skew = (k - 1) * 0.24f;
                        ThrowOne(owner, authority, i, focus, skew, FanMul);
                    }
                    if (ViewedOwner) {
                        ShakeViewer(1.2f);
                    }
        ...
        private void UpdateDissolve(KikasaDomainPlayer domain, bool authority) {
            ...
            for (int i = 0; i < handCount; i++) {
                ...
                if (lakeAlive && !dissolveSplashed[i] && lt >= 0 && handPos[i].Y >= lakeY) {
                    ...
                    SoundEngine.PlaySound(SoundID.SplashWeak with {
                        Volume = 0.5f, Pitch = -0.4f + i * 0.08f, MaxInstances = 3
                    }, handPos[i]);
                    if (ViewedOwner) {
            ...
            switch (State) {
                ...
                case StateRelay: {
                    ...
                    for (int i = 0; i < handCount; i++) {
                        ...
                        if (myTurn && p < ThrowWindup) {
                            ...
                            handSpin[i] = MathHelper.Lerp(handSpin[i], 0.5f, 0.2f);
                        }
                        else {
                ...
                case StateFan: {
                    ...
                    for (int i = 0; i < handCount; i++) {
                        ...
                        if (t < FanWindup + i * FanStagger) {
                            handSpin[i] = MathHelper.Lerp(handSpin[i], 0.55f, 0.15f);
                        }
                        handRot[i] += handSpin[i] + 0.02f;
                        handSpin[i] *= 0.92f;
                    }
        ...
        private void UpdateAmbient() {
            if (Main.dedServ
                || State is not (StateFollow or StateRelay or StateFan)) {
                return;
            }
        ...
        /// <summary>手上那枚的可见度：掷空 0，再凝渐回 1</summary>
        private float HeldVisible(int i)
            => 1f - MathHelper.Clamp(refill[i] / (float)RefillFrames, 0f, 1f);
        ...
        /// <summary>再凝期从全血水态收回稳态：新凝那枚湿一点</summary>
        private float HeldForm(int i)
            => MathHelper.Lerp(HandForm(i), 0.85f, MathHelper.Clamp(refill[i] / (float)RefillFrames, 0f, 1f));
        ...
        private void DrawBodies(SpriteBatch sb, Texture2D tex) {
            ...
            for (int i = 0; i < handCount; i++) {
                ...
                if (shaderOk) {
                    ...
                    form.Parameters["uForm"]?.SetValue(HeldForm(i));
                    form.Parameters["uDissolve"]?.SetValue(dissolve);
                    form.CurrentTechnique.Passes[0].Apply();
