...
namespace CalamityOverhaul.Content.LegendWeapon.MurasamaLegend
{
    internal class TooltipHandler
    {
        ...
        private static void ModifyMechBossSelect(int index, ref string newContent, ref string num) {
            ...
            //根据击败的Boss数量来确定提示
            //击败了所有两对Boss
            if (Downed7.Invoke() && Downed8.Invoke() && NPC.downedMechBoss1 && NPC.downedMechBoss3) {
                //全部击败，不需要提示
                return;
            }
            //击败了一对Boss的情况
            else if (NPC.downedMechBoss3 && Downed7.Invoke() && !NPC.downedMechBoss1 && !Downed8.Invoke()) {
                //只击败了机械统帅和硫磺火
                newContent = MuraText.GetTextValue("Subtest_Text1");  //提示击败毁灭者和渊灾
                num += "-2";
            }
            else if (NPC.downedMechBoss1 && Downed8.Invoke() && !NPC.downedMechBoss3 && !Downed7.Invoke()) {
                //只击败了毁灭者和渊灾
                newContent = MuraText.GetTextValue("Subtest_Text2");  //提示击败机械统帅和硫磺火
                num += "-2";
            }
            //一个都没击败
            else if (Level5) {
