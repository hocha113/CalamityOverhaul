...
namespace CalamityOverhaul.Content.LegendWeapon.HalibutLegend.FishSkills
{
    internal class FishDirt : FishSkill
    {
        ...
        public override bool? Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
            ...
            if (fishCount >= requiredFish && !HasActiveDirtBall(player)) {
        ...
        private void SpawnDirtFish(Player player) {
            float angle = Main.rand.NextFloat(MathHelper.TwoPi);
            float distance = Main.rand.NextFloat(200f, 350f);
            Vector2 spawnPos = player.Center + angle.ToRotationVector2() * distance;
            
            Vector2 velocity = (player.Center - spawnPos).SafeNormalize(Vector2.Zero) * Main.rand.NextFloat(3f, 6f);
        ...
        private int CountActiveDirtFish(Player player) {
            int count = 0;
        ...
        private bool HasActiveDirtBall(Player player) {
            for (int i = 0; i < Main.maxProjectiles; i++) {
                Projectile proj = Main.projectile[i];
                if (proj.active && proj.owner == player.whoAmI && 
        ...
        private void GatherAndShootDirtBall(Player player, EntitySource_ItemUse_WithAmmo source, 
            ...
            foreach (int index in fishIndices) {
                if (Main.projectile[index].ModProjectile is DirtFishFollower follower) {
                    follower.StartGathering();
                }
            }
            
            Vector2 shootDirection = targetVelocity.SafeNormalize(Vector2.Zero);
            
            Projectile.NewProjectile(
                source,
                player.Center + new Vector2(0, -80f),
                shootDirection * 16f,
                ModContent.ProjectileType<DirtBall>(),
                (int)(damage * (1.5f + HalibutData.GetDomainLayer() * 0.25f)),
                knockback * 1.8f,
                player.whoAmI,
                fishIndices.Count
            );
    ...
    internal class DirtFishFollower : ModProjectile
    {
        ...
        private const float SeparationRadius = 45f;
        private const float AlignmentRadius = 90f;
        private const float CohesionRadius = 120f;
        private const float MaxSpeed = 12f;
        private const float MaxForce = 0.6f;

        private const float PlayerFollowWeight = 2.5f;
        private const float SeparationWeight = 1.8f;
        private const float AlignmentWeight = 1.0f;
        private const float CohesionWeight = 1.2f;
        ...
        private void FollowingAI(Player owner) {
            ...
            Vector2 targetPos = owner.Center + playerRelativeVel * 8f;
            Vector2 toTarget = targetPos - Projectile.Center;
            float distanceToPlayer = toTarget.Length();
            
            Vector2 playerForce = Seek(targetPos, distanceToPlayer > 250f ? 1.5f : 1.0f);
            ...
            if (distanceToPlayer > 800f) {
                Projectile.Center = owner.Center + Main.rand.NextVector2Circular(150f, 150f);
        ...
        private void MovingToGatherAI(Player owner) {
            ...
            if (distance > 10f) {
                Vector2 desired = toGatherPoint.SafeNormalize(Vector2.Zero) * MaxSpeed * 1.8f;
                Projectile.velocity = Vector2.Lerp(Projectile.velocity, desired, 0.12f);
            } else {
                Projectile.velocity *= 0.9f;
            }
            
            if (GatherTimer > 30 && distance < 50f) {
        ...
        private void GatheringAI(Player owner) {
            ...
            if (GatherTimer < 25) {
                Vector2 repel = -toCenter.SafeNormalize(Vector2.Zero) * 8f;
                Projectile.velocity = Vector2.Lerp(Projectile.velocity, repel, 0.15f);
            }
            else if (GatherTimer < 60) {
                Projectile.velocity *= 0.88f;
                
                Vector2 offset = new Vector2(
                    (float)Math.Cos(wigglePhase) * 15f,
                    (float)Math.Sin(wigglePhase * 0.7f) * 15f
                );
                Vector2 orbitPos = gatherCenter + offset;
                Vector2 toOrbit = orbitPos - Projectile.Center;
                Projectile.velocity += toOrbit * 0.02f;
            }
            else {
        ...
        private void ConvergingAI(Player owner) {
            ...
            if (distance > 3f) {
                Vector2 desired = toCenter.SafeNormalize(Vector2.Zero) * MaxSpeed * 2f;
        ...
        private void ApplySteering(Vector2 force) {
            ...
            Projectile.velocity = Vector2.Lerp(Projectile.velocity, boidVelocity, 0.3f);
    ...
    internal class DirtBall : ModProjectile
    {
        ...
        private ref float RollingTime => ref Projectile.ai[2];
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            for (int i = 0; i < fishToDraw; i++) {
                ...
                float radius = Projectile.width * 0.35f * ballSize;
