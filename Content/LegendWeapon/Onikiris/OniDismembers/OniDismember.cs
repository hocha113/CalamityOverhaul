...
namespace CalamityOverhaul.Content.LegendWeapon.Onikiris.OniDismembers
{
    ...
    internal class OniDismember : ICWRLoader
    {
        ...
        private static DismemberEntry CreateEntry(NPC npc, int duration) {
            ...
            if (!Main.dedServ) {
                ...
                entry.SnapWidth = Math.Clamp(w & ~1, 64, 1600);
                entry.SnapHeight = Math.Clamp(h & ~1, 64, 1600);
        ...
        /// <summary>保留 dot(v-p, n)*side ≥ 0 半平面的凸多边形裁剪</summary>
        private static List<Vector2> ClipHalfPlane(Vector2[] poly, Vector2 p, Vector2 n, float side) {
        ...
        private static Vector2 PolyCentroid(List<Vector2> poly) {
            float signedArea = 0f;
