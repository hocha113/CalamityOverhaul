...
namespace CalamityOverhaul.Content.Scenarios.Kiame.Gen
{
    ...
    //gen 线程单线程使用，静态计数器供收尾日志。镜像 KiyumeTileBrush，不引用
    internal static class KiameTileBrush
    {
        ...
        //平台直写：frameX 留给收尾帧修，frameY 即平台样式（0=原木）
        internal static void SetPlatform(int x, int y, short frameY = 0) {
            if (!WorldGen.InWorld(x, y)) {
                return;
            }
            Tile tile = Main.tile[x, y];
