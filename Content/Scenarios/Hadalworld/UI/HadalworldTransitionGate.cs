...
{
    /// <summary>
    /// 进入/退出的「先遮再冻」压黑门(镜像 DungeonworldTransitionGate)<br/>
    /// SLib 时序:BeginEntering 当帧设 gameMenu=true 后存档/卸载/生成全在后台 Task,
    /// 主线程理论上持续绘制加载屏,但大世界的分配与 GC 会造成主线程长帧,暴露未经修饰的冻结黑。<br/>
    /// 本门在真正调 SubworldSystem.Enter/Exit 之前先播 0.45s 客户端压黑(没入水面意象),
    /// 全黑再多呈现一帧后才提交过渡,把「突兀冻结黑」变成「有意为之的入水压黑」;
    /// 联机客户端提交后 gameMenu 由服务器回包才翻转,期间黑幕持续保持(带超时撤防)
    /// </summary>
    internal class HadalworldTransitionGate : ModSystem
    {
        ...
        internal static void Begin(bool enterDirection, Func<bool> commitAction) {
            ...
            HadalworldTransitionLog.NewEpoch(enterDirection
                ? "EnterWorld() 调用,压黑渐入开始" : "ExitWorld() 调用,压黑渐入开始");
            //没入水面的落水声,后续过带水涌由加载屏负责
            SoundEngine.PlaySound(SoundID.Splash with { Pitch = -0.5f, Volume = 0.55f });
        }
        ...
        public override void PostUpdateEverything() {
            ...
            if (phase == 1) {
                ...
                var doCommit = commit;
                commit = null;
                HadalworldTransitionLog.Mark("压黑完成,提交过渡");
                bool ok = doCommit?.Invoke() ?? false;
                if (!ok) {
                    HadalworldTransitionLog.Mark("过渡提交被拒绝,撤幕放行");
                    phase = 3;
                    return;
                }
                HadalworldTransitionLog.Mark($"过渡已受理 gameMenu={Main.gameMenu}");
            ...
            if (phase == 2) {
                ...
                if (awaitT >= TakeoverTimeoutSeconds) {
                    HadalworldTransitionLog.Mark("等待 SLib 接管超时,撤幕放行");
                    phase = 3;
                }
                return;
            }
        ...
        public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers) {
            ...
            layers.Add(new LegacyGameInterfaceLayer(
                "CWRMod: Hadalworld Transition Gate",
                delegate {
    ...
    /// <summary>
    /// [HadalworldTransition] 时间线:一次过渡一个纪元,只记 Enter/提交/DrawSetup 首帧(及偶发长帧)
    /// </summary>
    internal static class HadalworldTransitionLog
    {
        ...
        internal static void Mark(string evt) {
            ...
            CWRMod.Instance?.Logger.Info($"[HadalworldTransition] +{delta}ms {evt}");
        }
    }
}
