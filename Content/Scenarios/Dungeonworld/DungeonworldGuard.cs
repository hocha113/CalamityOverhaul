...
{
    //主世界保护快照(镜像CybCourseWorldGuard,F34)
    //Enter前Snapshot,回主世界RestoreOnReturn
    //绑_sourceWorldId,回到不同世界整组丢弃
    internal static class DungeonworldGuard
    {
        ...
        private static void RestoreTownNPCs() {
            ...
            foreach (NpcEntry entry in _npcSnapshot) {
                ...
                NPC.NewNPC(new EntitySource_Misc("Dungeonworld_NPCRestore"), entry.X, entry.Y, entry.Type);
            }
            _npcSnapshot = null;
        }
    }
