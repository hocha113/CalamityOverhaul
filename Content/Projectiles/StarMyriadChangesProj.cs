            ...
            if (Text.Length > 0) {
                for (int i = 0; i < 33; i++) {
                    ...
                    PRTLoader.AddParticle(new PRT_StarPulseRing(Projectile.Center + Main.rand.NextVector2Unit() * Main.rand.Next(13, 330)
                        , Vector2.Zero, VaultUtils.MultiStepColorLerp(Main.rand.NextFloat(1), HeavenfallLongbow.rainbowColors), 0.05f * slp, 0.8f * slp, 8));
                }
                ...
                for (int i = 0; i < rands.Count; i++) {
                    ...
                    CombatText.NewText(rectangle, VaultUtils.MultiStepColorLerp(Main.rand.NextFloat(1), HeavenfallLongbow.rainbowColors), Text[rands[i]], true);
                }
