...
{
    internal class UniversalGenesisHeld : BaseFeederGun
    {
        ...
        public override void FiringShoot() {
            if (AmmoTypes == ProjectileID.Bullet) {
                AmmoTypes = CWRID.Proj_UniversalGenesisStarcaller;
            }
            ...
            for (int i = 0; i < 7; i++) {
                pos += VaultUtils.RandVr(134);
                Vector2 vr = pos.To(Main.MouseWorld).UnitVector() * ShootSpeedModeFactor * Main.rand.NextFloat(1.9f, 2.6f);
                Projectile.NewProjectile(Source, pos, vr, CWRID.Proj_UniversalGenesisStar, WeaponDamage / 2, WeaponKnockback / 2, Owner.whoAmI, i, 1);
            }
        }
    }
}
