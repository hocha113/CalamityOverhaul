...
namespace CalamityOverhaul.Content.Projectiles.Weapons.Melee.AstralProj
{
    internal class AstralBall : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
            target.AddBuff(CWRID.Buff_AstralInfectionDebuff, 240);
            ...
            for (float angle = 0f; angle < MathHelper.TwoPi; angle += 0.01f) {
                ...
                Dust d = Dust.NewDustPerfect(target.Center, Main.rand.NextBool() ? CWRID.Dust_AstralBlue : CWRID.Dust_AstralOrange, velocity);
                d.customData = 0.025f;
                d.scale = 1.25f;
                d.noLight = false;
            }
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) {
            ...
            if (multiplier > 1.5f) {
                ...
                for (float angle = 0f; angle < MathHelper.TwoPi; angle += var) {
                    blue = !blue;
                    Vector2 velocity = angle.ToRotationVector2() * (2f + (float)(Math.Sin(angleStart + angle * 3f) + 1) * 2.5f) * Main.rand.NextFloat(0.95f, 1.05f);
                    Dust d = Dust.NewDustPerfect(target.Center, blue ? CWRID.Dust_AstralBlue : CWRID.Dust_AstralOrange, velocity);
                    d.customData = 0.025f;
        ...
        public override void OnKill(int timeLeft) {
            ...
            for (int i = 0; i < 60; i++) {
                ...
                Dust d = Dust.NewDustPerfect(pos, CWRID.Dust_AstralBlue, angleVec * Main.rand.NextFloat(2f, 4f));
                d.customData = true;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            CWRRef.DrawAfterimagesCentered(Projectile, ProjectileID.Sets.TrailingMode[Projectile.type], lightColor, 12);
            return false;
        }
    }
}
