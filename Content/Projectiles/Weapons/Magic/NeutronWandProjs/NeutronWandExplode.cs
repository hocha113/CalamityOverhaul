    ...
    internal class NeutronWandExplode : ModProjectile, IDrawWarp
    {
        ...
        private void SpanStar(Vector2 offset) {
            for (int i = 0; i < 4; i++) {
                ...
                for (int j = 0; j < 13; j++) {
                    BaseParticle spark = new DRK_HeavenfallStar(Projectile.Center + offset
        ...
        public override void AI() {
            if (Projectile.ai[2] == 0) {
                for (int j = 0; j < 122; j++) {
                    BaseParticle spark2 = new DRK_HeavenfallStar(Projectile.Center + new Vector2(Main.rand.Next(-16, 16), -700)
