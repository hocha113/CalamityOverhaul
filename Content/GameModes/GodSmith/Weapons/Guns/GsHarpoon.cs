...
namespace CalamityOverhaul.Content.GameModes.GodSmith.Weapons.Guns
{
    ...
    internal class GsHarpoon : GsMagazineScheme
    {
        ...
        //==================== 后坐姿态：掷叉沉手（差分，见 GsGunRecoil） ====================

        public override void GsUseStyle(Item item, Player player, Rectangle heldItemFrame)
            => GunKickStyle(player, 2.5f, 0.1f);
    }
    ...
    internal class GsHarpoonSpearProj : ModProjectile
    {
        ...
        /// <summary>线状一笔：原版锁链贴图从手到叉头逐节平铺（镜像原版钩爪链画法），再画叉头本体</summary>
        public override bool PreDraw(ref Color lightColor) {
            ...
            if (dist > chain.Height) {
                Vector2 dir = delta / dist;
                float rot = dir.ToRotation() - MathHelper.PiOver2;
                int links = Math.Min((int)(dist / chain.Height), 64);
                for (int i = 0; i < links; i++) {
                    Vector2 pos = from + dir * (chain.Height * (i + 0.5f));
                    Color c = Lighting.GetColor((int)(pos.X / 16f), (int)(pos.Y / 16f));
                    Main.EntitySpriteDraw(chain, pos - Main.screenPosition, null, c, rot,
                        chain.Size() / 2f, 1f, SpriteEffects.None, 0);
                }
            }
