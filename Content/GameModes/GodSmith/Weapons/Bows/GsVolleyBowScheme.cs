...
namespace CalamityOverhaul.Content.GameModes.GodSmith.Weapons.Bows
{
    ...
    /// 充能：每次射击积攒 <see cref="ChargePerShot"/>，满 100 下一发自动成齐射。<br/>
    ...
    internal abstract class GsVolleyBowScheme : GodSmithScheme
    {
        ...
        /// <summary>齐射帧弦响（owner 端个人反馈，编队箭本体跨端可见）</summary>
        protected static void VolleyFX(Vector2 position) {
        ...
        /// <summary>处决触发的通用音效（攻击方端个人反馈；跨端表现由 rider 弹幕承担）</summary>
        protected static void ExecuteFX(NPC target) {
            if (VaultUtils.isServer) {
                return;
            }
