...
{
    /// <summary>
    /// 养蜂箱面板:空瓶仓+产出仓+灌装进度条+启停+电力表盘,
    /// 笔刷与材质在 <see cref="IndustrialTerminalRenderer"/>
    /// </summary>
    internal class ApiaryUI : UIHandle, ILocalizedModType
    {
        public string LocalizationCategory => "UI.Apiary";
        ...
        private static Color Accent => Apiary.Tint;
        ...
        public static ApiaryUI Instance => UIHandleLoader.GetUIHandleOfType<ApiaryUI>();

        internal ApiaryTP Station;
        ...
        private readonly Rectangle[] bottleRects = new Rectangle[ApiaryTP.BottleSlotCount];
        private readonly Rectangle[] produceRects = new Rectangle[ApiaryTP.ProduceSlotCount];
        private Rectangle progressRect;
        private Rectangle toggleBtn;
        private Vector2 gaugeCenter;
        private Rectangle gaugeRect;

        private int hoveringBottle = -1;
        ...
        protected static LocalizedText BottleLabel;
        ...
        protected static LocalizedText StatusWorkingBoost;
        ...
        protected static LocalizedText BottleHint;
        ...
        public override void SetStaticDefaults() {
            TitleText = this.GetLocalization(nameof(TitleText), () => "养蜂箱");
            BottleLabel = this.GetLocalization(nameof(BottleLabel), () => "空瓶仓");
            ...
            StatusWorking = this.GetLocalization(nameof(StatusWorking), () => "酿蜜中");
            StatusWorkingBoost = this.GetLocalization(nameof(StatusWorkingBoost), () => "蜂群活跃");
            StatusIdle = this.GetLocalization(nameof(StatusIdle), () => "歇工");
        ...
        public override void Update() {
            ...
            hoveringBottle = -1;
            hoveringProduce = -1;
            for (int i = 0; i < bottleRects.Length; i++) {
                if (bottleRects[i].Contains(mouse) && !isDragging) {
                    hoveringBottle = i;
                    break;
                }
            }
            ...
            if (keyLeftPressState == KeyPressState.Pressed) {
                ...
                else if (hoveringBottle >= 0) {
                    HandleBottleClick(hoveringBottle);
                }
            ...
            //背景区拖拽,避开控件
            bool overControl = hoveringToggle || hoveringBottle >= 0 || hoveringProduce >= 0 || closeRect.Contains(mouse);
        ...
        private void ComputeLayout() {
            ...
            for (int i = 0; i < bottleRects.Length; i++) {
                bottleRects[i] = new Rectangle(panelRect.X + 26 + i * (SlotSize + SlotGap), panelRect.Y + 76, SlotSize, SlotSize);
            }
            for (int i = 0; i < produceRects.Length; i++) {
                produceRects[i] = new Rectangle(panelRect.X + 26 + i * (SlotSize + SlotGap),
                    panelRect.Y + 164, SlotSize, SlotSize);
            }
            progressRect = new Rectangle(panelRect.X + 26, panelRect.Y + 230, 4 * SlotSize + 3 * SlotGap, 10);

            toggleBtn = new Rectangle(panelRect.X + 300, panelRect.Y + 76, 110, 30);
            gaugeCenter = new Vector2(panelRect.X + 372, panelRect.Y + 200);
            gaugeRect = new Rectangle((int)gaugeCenter.X - 32, (int)gaugeCenter.Y - 32, 64, 64);
        }

        /// <summary>瓶仓交互:只收空玻璃瓶;本地改+SendData推送(客户端权威的UI编辑模型)</summary>
        private void HandleBottleClick(int index) {
            Item slotItem = Station.Bottles[index];
            Item mouseItem = Main.mouseItem;

            if (mouseItem.IsAir) {
                if (slotItem != null && !slotItem.IsAir) {
                    Main.mouseItem = slotItem.Clone();
                    Station.Bottles[index] = new Item();
            ...
            if (!ApiaryTP.IsEmptyBottle(mouseItem)) {
                SoundEngine.PlaySound(SoundID.MenuClose with { Volume = 0.4f, Pitch = -0.2f });
                return;
            }

            if (slotItem == null || slotItem.IsAir) {
                Station.Bottles[index] = mouseItem.Clone();
                mouseItem.TurnToAir();
                SoundEngine.PlaySound(SoundID.Grab);
                Station.SendData();
            }
        ...
        public override void SaveUIData(TagCompound tag) {
            tag["ApiaryUI_DrawPos_X"] = DrawPosition.X;
            tag["ApiaryUI_DrawPos_Y"] = DrawPosition.Y;
        }
        ...
        private void DrawSlots(SpriteBatch sb) {
            ...
            Utils.DrawBorderString(sb, BottleLabel.Value,
                new Vector2(panelRect.X + 26, panelRect.Y + 54), TextDim * alpha, 0.62f);
            for (int i = 0; i < bottleRects.Length; i++) {
                IndustrialTerminalRenderer.DrawSocket(sb, bottleRects[i], alpha, hoveringBottle == i ? 1f : 0f, 0f);
                DrawSlotItem(sb, Station.Bottles[i], bottleRects[i], alpha);
            }
            ...
            //灌装进度条:环境加成时染成更亮的蜜色
            float progress = MathHelper.Clamp(Station.BrewProgress / ApiaryTP.CycleTicks, 0f, 1f);
            Color barColor = Station.EnvBonus ? Color.Lerp(Accent, Color.White, 0.25f) : Accent;
            IndustrialTerminalRenderer.DrawTickBar(sb, progressRect, progress, barColor, alpha);
        }
        ...
        private void DrawStatusColumn(SpriteBatch sb) {
            ...
            else if (Station.MachineData.UEvalue < ApiaryTP.BrewCost) {
                state = StatusNoPower.Value;
                lampColor = WarnRed;
                lampBright = MathF.Sin(animTimer * 5f) * 0.35f + 0.55f;
            }
            else if (Station.IsWorking) {
                state = Station.EnvBonus ? StatusWorkingBoost.Value : StatusWorking.Value;
                lampColor = Accent;
        ...
        private void DrawHoverTips(SpriteBatch sb) {
            ...
            if (hoveringBottle >= 0) {
                Item item = Station.Bottles[hoveringBottle];
                if (item != null && !item.IsAir) {
                    Main.HoverItem = item.Clone();
                    Main.hoverItemName = item.Name;
                }
                else {
                    ShowTip(sb, BottleHint.Value);
                }
            }
            else if (hoveringProduce >= 0) {
