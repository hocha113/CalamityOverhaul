...
{
    /// <summary>
    /// 蘑菇农场机面板:产出仓+启停+电力表盘,无种子仓(蘑菇靠地表扫描播种),
    /// 笔刷与材质在 <see cref="IndustrialTerminalRenderer"/>
    /// </summary>
    internal class MushroomFarmerUI : UIHandle, ILocalizedModType
    {
        public string LocalizationCategory => "UI.MushroomFarmer";
        ...
        private static Color Accent => MushroomFarmer.Tint;
        ...
        public static MushroomFarmerUI Instance => UIHandleLoader.GetUIHandleOfType<MushroomFarmerUI>();

        internal MushroomFarmerTP Station;
        ...
        private readonly Rectangle[] produceRects = new Rectangle[MushroomFarmerTP.ProduceSlotCount];
        ...
        public override void SetStaticDefaults() {
            TitleText = this.GetLocalization(nameof(TitleText), () => "蘑菇农场机");
            ...
            StatusWorking = this.GetLocalization(nameof(StatusWorking), () => "培育中");
        ...
        public void Interactive(MushroomFarmerTP tp) {
            if (tp == null) {
                return;
            }
        ...
        public override void Update() {
            ...
            //背景区拖拽,避开控件
            bool overControl = hoveringToggle || hoveringProduce >= 0 || closeRect.Contains(mouse);
        ...
        private void ComputeLayout() {
            ...
            for (int i = 0; i < produceRects.Length; i++) {
                ...
                produceRects[i] = new Rectangle(panelRect.X + 26 + col * (SlotSize + SlotGap),
                    panelRect.Y + 82 + row * (SlotSize + SlotGap), SlotSize, SlotSize);
            }

            toggleBtn = new Rectangle(panelRect.X + 300, panelRect.Y + 76, 110, 30);
            gaugeCenter = new Vector2(panelRect.X + 372, panelRect.Y + 172);
            gaugeRect = new Rectangle((int)gaugeCenter.X - 32, (int)gaugeCenter.Y - 32, 64, 64);
        }

        /// <summary>产出槽只出不进:空手点击取出整叠;客户端权威编辑,改完推送</summary>
        ...
        public override void SaveUIData(TagCompound tag) {
            tag["MushroomFarmerUI_DrawPos_X"] = DrawPosition.X;
            tag["MushroomFarmerUI_DrawPos_Y"] = DrawPosition.Y;
        }
        ...
        private void DrawSlots(SpriteBatch sb) {
            ...
            Utils.DrawBorderString(sb, ProduceLabel.Value,
                new Vector2(panelRect.X + 26, panelRect.Y + 58),
        ...
        private void DrawStatusColumn(SpriteBatch sb) {
            ...
            //状态灯
            Vector2 lampPos = new(panelRect.X + 310, panelRect.Y + 126);
            string state;
            Color lampColor;
            float lampBright;
            if (!Station.Enabled) {
            ...
            else if (Station.MachineData.UEvalue < MushroomFarmerTP.PlantCost) {
