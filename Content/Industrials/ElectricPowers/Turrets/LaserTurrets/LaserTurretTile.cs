...
{
    /// <summary>激光塔瓦片,3x5;复用特斯拉塔瓦片贴图(常规形态帧)并施加猩红色调</summary>
    internal class LaserTurretTile : ModTile
    {
        ...
        public override void SetStaticDefaults() {
            ...
            AddMapEntry(new Color(190, 80, 80), VaultUtils.GetLocalizedItemName<LaserTurret>());
        ...
        public override void MouseOver(int i, int j) {
            Main.LocalPlayer.SetMouseOverByTile(ModContent.ItemType<LaserTurret>());
        }

        public override bool RightClick(int i, int j) {
            if (!TileProcessorLoader.AutoPositionGetTP<LaserTurretTP>(i, j, out var tp)) {
                return false;
            }
            tp.RightEvent();
            return true;
        ...
        public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b) {
            ...
            if (!TileProcessorLoader.ByPositionGetTP(point, out LaserTurretTP tp)) {
                return;
            }
            if (tp.GlowIntensity > 0.05f) {
                r = 0.40f * tp.GlowIntensity;
                g = 0.10f * tp.GlowIntensity;
                b = 0.10f * tp.GlowIntensity;
            }
        }
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch) {
            ...
            //共用特斯拉塔贴图,乘上系列色调区分机种
            Color drawColor = Lighting.GetColor(i, j).MultiplyRGB(LaserTurret.Tint);
            ...
            Color glowColor = LaserTurret.Tint * (0.3f + tp.GlowIntensity * 0.7f);
