...
{
    /// <summary>火焰喷射塔瓦片,3x5;复用特斯拉塔瓦片贴图(常规形态帧)并施加橙红色调</summary>
    internal class FlameTurretTile : ModTile
    {
        ...
        public override void SetStaticDefaults() {
            ...
            AddMapEntry(new Color(196, 110, 60), VaultUtils.GetLocalizedItemName<FlameTurret>());
        ...
        public override bool CreateDust(int i, int j, ref int type) {
            Dust.NewDust(new Vector2(i, j) * 16f, 16, 16, DustID.Torch);
            return false;
        }
        ...
        public override void MouseOver(int i, int j) {
            Main.LocalPlayer.SetMouseOverByTile(ModContent.ItemType<FlameTurret>());
        }

        public override bool RightClick(int i, int j) {
            if (!TileProcessorLoader.AutoPositionGetTP<FlameTurretTP>(i, j, out var tp)) {
                return false;
            }
            tp.HandleRightClick();
            return true;
        ...
        public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b) {
            ...
            if (!TileProcessorLoader.ByPositionGetTP(point, out FlameTurretTP tp)) {
                return;
            }
            if (tp.GlowIntensity > 0.05f) {
                r = 0.42f * tp.GlowIntensity;
                g = 0.20f * tp.GlowIntensity;
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch) {
            ...
            //共用特斯拉塔贴图,乘上系列色调区分机种
            Color drawColor = Lighting.GetColor(i, j).MultiplyRGB(FlameTurret.Tint);

            if (!tp.AttackPattern || tp.GlowIntensity < 0.05f) {
                drawColor.R /= 2;
                drawColor.G /= 2;
                drawColor.B /= 2;
                drawColor.A = 255;
            ...
            Color glowColor = FlameTurret.Tint * (0.3f + tp.GlowIntensity * 0.7f);
